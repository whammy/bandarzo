...
class WorkoutsController < ApplicationController
  ...
  DEFAULT_STAGES = ["Warm Up", "Main Set", "Cool Down" ]
  ...
  def new
    ...
    @workout = Workout.new
    DEFAULT_STAGES.each do |s|
      @workout.stages.build(:name => s)
    end
  ...
  def duplicate
    @workout = Workout.find(params[:id]).make_copy
    
    respond_to do |format|
      format.html { render :action => "new" } # new.html.erb
      format.xml  { render :action => "new" ,:xml => @workout }
    end
  end
